...
def template_match(template_img, orig_image, distance=0.7, min_match_count=4):
    try:
        ...
        if len(good) > min_match_count:
            ...
            print(
                "No se encontraron suficientes matches - {}/{}".format(
    ...
    for keypoint in json_template_data:
        ...
        if not (keypoint == "extension" or keypoint == "name"):
            ...
            if retry_ocr and keypoint not in json_img_data:
                img_aux, crop_ocr = get_ocr_data(
                    Image_To_Convert[
                        y_inicial_template:y_final_template,
                        x_inicial_template:x_final_template,
                    ]
                )
...
def manual_adjust_image_rotation(Image_To_Convert):
    def on_change(value):
        ...
        print(
            str(adaptiveThreshold_1)
            + " "
            + str(adaptiveThreshold_2)
            + " "
            + str(cannyEdges_1)
            + " "
            + str(cannyEdges_2)
        ...
        lines = cv2.HoughLinesP(
            ...
            rho=1,
            theta=1 * math.pi / 180,
            threshold=linesThreshold_1,
            minLineLength=10,
            maxLineGap=linesThreshold_2,
        )
        ...
        imS = np.hstack(
            (
                cv2.resize(
                    grey_3_channel,
                    (
                        round(grey_3_channel.shape[1] * 600 / grey_3_channel.shape[0]),
                        600,
                    ),
                ),
                cv2.resize(
                    finalImage,
                    (round(finalImage.shape[1] * 600 / finalImage.shape[0]), 600),
                ),
            )
        )
    ...
    imS = cv2.resize(
        Image_To_Convert,
        (round(Image_To_Convert.shape[1] * 600 / Image_To_Convert.shape[0]), 600),
    )  # Resize image
    ...
    try:
        ...
        angle_corrections = [
            x
            for x in angle_corrections
            if (
                x >= (np.mean(angle_corrections) - np.std(angle_corrections))
                and x <= (np.mean(angle_corrections) + np.std(angle_corrections))
            )
        ]
...
def adjust_image_orientation(Image_To_Convert):
    ...
    try:
        ...
        Image.fromarray(Image_To_Convert).save(temp_file, dpi=(70, 70))

        # Get OCR Orientation Data
        new_data = tess.image_to_osd(temp_file)

        # remove temp file
        os.remove(temp_file)

        # Get Orientation Angle
        OCR_Orientation_Angle = int(re.search("(?<=Rotate: )\d+", new_data).group(0))
...
def get_ocr_data(
    Image_To_Convert, min_conf=0, deleted_words=["", " "], draw_rectangles=False
):
    # Get OCR Data from Image
    imagedataocr = tess.image_to_data(Image_To_Convert, output_type=Output.DICT)
    ...
    for i in range(n_boxes):
        if int(float(imagedataocr["conf"][i])) > min_conf and (
            imagedataocr["text"][i] not in deleted_words
        ):
            (x, y, w, h) = (
                imagedataocr["left"][i],
                imagedataocr["top"][i],
                imagedataocr["width"][i],
                imagedataocr["height"][i],
            )
            if draw_rectangles:
...
def get_ocr_data_azure(
    Bytes_Image_To_Convert,
    Image_To_Convert,
    min_conf=0,
    deleted_words=["", " "],
    ...
    # Verify all OCR Data and delete Confidences = -1 or deleted words
    for i in range(n_boxes):
        if int(100 * imagedataocr["conf"][i]) > min_conf and (
            imagedataocr["text"][i] not in deleted_words
        ):
            (x, y, w, h) = (
                imagedataocr["left"][i],
...
def adjust_image_size(Image_To_Convert, Max_Size=1700):
    ...
    if img_width > img_height:
        scale_percent = (
            Max_Size * 100 / img_width
        )  # Percent of original size from width
    else:
        scale_percent = (
            Max_Size * 100 / img_height
        )  # Percent of original size from height

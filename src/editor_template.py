...
modo_dict = {0: {'txt_modo': "Giro", 'color_modo': (0, 0, 0)}, 1: {'txt_modo': "Recorte", 'color_modo': (0, 255, 0)}, 2: {'txt_modo': "Numero", 'color_modo': (255, 0, 0)}, 3: {'txt_modo': "Fecha", 'color_modo': (0, 0, 255)}, 4: {'txt_modo': "Total", 'color_modo': (255, 255, 0)}, 5: {'txt_modo': "Cliente", 'color_modo': (0, 255, 255)}}
#General Variables
drawing = False
angulo = 0.0
modo = 0
...
def drawRectangle(action, x, y, flags, *userdata):
    ...
    # When left mouse button is released, mark bottom right corner
        ...
        drawing = False
        # Draw the rectangle
        cv2.rectangle(resize_image_editable,
                      top_left_corner[0], bottom_right_corner[0], modo_dict[modo]['color_modo'], 1, 8)
        if modo > 0:
            cv2.putText(resize_image_editable, modo_dict[modo]['txt_modo'], top_left_corner[0],   font,
                        fontScale,
...
for root, dirs, files in os.walk(Templates_Files_Directory, topdown=False):
    for filename in files:
        ...
        if folder in folders_to_process and extension in accepted_file_extension and not os.path.isfile(template_data_file_name):
            ...
            if original_image.shape[1] > original_image.shape[0]:
                factor = 700/original_image.shape[1]  # Percent of original size from width
            else:
            ...
            while k != 27:
                ...
                if modo == 0 and (k == 97):
                    angulo=angulo + 0.1
                    resize_image_editable=ndimage.rotate(resize_image, angulo, cval = 255)
                    cv2.imshow("Window", resize_image_editable)
                    ...
                    if modo == 0:
                        original_image=ndimage.rotate(original_image, angulo, cval = 255)
                        ...
                        if len(top_left_corner) > 0 and len(bottom_right_corner) > 0:
                            if modo == 1:
                                original_image=original_image[int(1/factor*top_left_corner[0][1]):int(1/factor*bottom_right_corner[0][1]), int(
                                    1/factor*top_left_corner[0][0]):int(1/factor*bottom_right_corner[0][0])]
                                resize_image=cv2.resize(
                                    original_image, None, fx = factor, fy = factor)
                                resize_image_editable=resize_image
                            else:
                                modificaciones[modo_dict[modo]['txt_modo']]=[(int(1/factor*top_left_corner[0][0]), int(1/factor*top_left_corner[0][1])), (int(
                                    1/factor*bottom_right_corner[0][0]), int(1/factor*bottom_right_corner[0][1]))]
            ...
            for key in modificaciones:
                cv2.rectangle(resize_image_editable,
                            modificaciones[key][0], modificaciones[key][1], (255, 0, 0), 1, 8)
                img_data[key] = [modificaciones[key][0], modificaciones[key][1]]
                cv2.putText(resize_image_editable, key, modificaciones[key][0],   font,
                            fontScale,
                            (255,0,0),
                            thickness,
                            lineType)
